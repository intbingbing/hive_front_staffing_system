<template lang="pug">
    section
        el-dialog(title="批量信息修改" :visible.sync="showDialog" width="40%" :show-close="false")
            el-form(:model="confirmBatchNodeData" size="small" label-position="right"  label-width="80px")
                el-row
                    el-col(:span="18")
                        el-form-item(label="属性")
                            el-select(v-model="confirmBatchNodeData.association_is_department")
                                el-option(v-for="val of this.$store.state.propertySelect"
                                    :key="val.value" :label="val.label" :value="val.value")
                        el-form-item(label="所在部门")
                            el-select(v-model="confirmBatchNodeData.association_pid")
                                el-option(v-for="val of departmentInfo"
    ...
    export default {
        ...
        data() {
            return {
                showDialog: false,
                batchNodeArr:[],
                confirmBatchNodeData:{},
            };
        },
        methods:{
            changeDialogStatus(nodeArr){
                //console.log(node);
//                console.log(typeof node);
                this.showDialog = true;
                this.batchNodeArr = JSON.parse(JSON.stringify(nodeArr));
            },
            async submitUpdate(){
